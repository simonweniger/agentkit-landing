...
AgentKit allows users to do question answering over documents by attaching a Document to an Agent. Documents are files, such as PDF, TXT, images, Markdown etc. that can be ingested by passing a publicly available URL to the AgentKit API. AgentKit splits these documents into smaller chunks and stores them in a vector database for use downstream. This approach has some downsides when working with tabular data. We are looking into how to best approach this problem. This is a community-made SwiftSDK for the Superagen LLM Framework. You need to have a superagent API Key in order to use the SDK.
...
This guide will get you all set up and ready to use AgentKit Tools. AgentKit allows users to configure their Agents using tools. A tool is a Api or piece of code that helps the LLM to validate its response. It can be an api that AgentKit calls to get real time information or a Zapier integration. Tools are helpfull to give LLMs access to real time and real world data while also reducing haluzination
